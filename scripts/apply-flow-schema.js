...
// Script to apply Flow Command Tracking schema to Supabase
// This creates the flow_logs table and related functions
...
async function applyFlowSchema() {
  try {
    console.log('🌊 Setting up Flow Command Tracking...');
    
    // Read the SQL schema file
    const schemaPath = path.join(__dirname, '..', 'supabase', 'flow_tracking.sql');
    ...
    console.log('\n🧪 Testing Flow logging function...');
    const testResult = await supabase.rpc('log_flow_command', {
      p_wallet_address: 'test_wallet_script',
      p_username: 'script_test',
      p_access_level: 'BETA',
    ...
    console.log('\n🎉 Flow Command Tracking Schema setup complete!');
    console.log('👉 You can now test "flow" command in your app terminal');
